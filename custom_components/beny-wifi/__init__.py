...
async def async_setup(hass: HomeAssistant, config: dict):
    """Set up the EV Charger integration using YAML (not supported here)."""
    return True
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    _LOGGER.info(f"Setting up EV Charger with IP {ip_address} and port {port}")
    ...
    async def fetch_state():
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                ...
                update_command = bytes.fromhex(COMMANDS["UPDATE"])  # Replace with actual update command
                s.sendto(update_command, (ip_address, port))

                response, _ = s.recvfrom(1024)
                _LOGGER.info(f"Received state update: {response.hex()}")
                return response.hex()
    ...
    async def send_command(command):
        """Send a command to the EV charger."""
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                ...
                _LOGGER.info(f"Sent {command} command to {ip_address}:{port}")

                response, _ = s.recvfrom(1024)
                _LOGGER.info(f"Received response: {response.hex()}")
                return response
        except Exception as e:
    ...
    async def start_charge_service(call):
        ...
        if response:
            _LOGGER.info("Start command executed successfully.")
        else:
    ...
    async def stop_charge_service(call):
        ...
        if response:
            _LOGGER.info("Stop command executed successfully.")
        else:
            _LOGGER.warning("Failed to confirm stop command.")

    # Register sensors
    async_add_entities([
        BenyStateSensor("EV Charger State", fetch_state),
